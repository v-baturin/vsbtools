...
def load_from_materials_project(elements, message=None):
...
def load_from_oqmd(elements, message=None):
...
def load_from_uspex_calc_folders(calcfolds_path: Path, stage=None, message=None, elements=None):
    ...
    message = message or f"Parsed from uspex CalcFolds" +  (f' with elements={elements}' if elements else '.')
    return df2ds(df, message=message)
...
def load_from_uspex_goodstructures(goodstructures_path: Path, message=None, elements=None):
    ...
    df = uspex_client.query(elements=elements)
    message = message or f"Parsed from uspex goodStructures" +  (f' with elements={elements}' if elements else '.')
    return df2ds(df, message=message)
...
def load_mattersim_estimated_set(csv_path: Path, poscars_base_path: Path, message=None):
    ...
    message = message or f"Dataset with Energies estimated by Mattersim and structures from {poscars_base_path}"
    return df2ds(df, message=message)