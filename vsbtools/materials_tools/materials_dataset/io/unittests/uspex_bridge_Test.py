...
class USPEXBridge_Test(unittest.TestCase):
    ...
    def setUp(self):
        self.two_systems = StructureDatasetIO(PATH_WITH_DATASETS/"two_systems_MgAlO").load_from_directory()
        self.ub = USPEXBridge(elements={'Mg', 'Al', 'O'}, legacy=True)


    def test_distance(self):
        self.assertAlmostEqual(self.ub.fp_dist(*self.two_systems), 0.1725, places=4)
