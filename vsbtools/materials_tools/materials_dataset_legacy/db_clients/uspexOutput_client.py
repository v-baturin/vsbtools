...
class USPEXOutputClient:
    ...
    def query(self, elements: Iterable[str]) -> pd.DataFrame:
        if self.mode == 'goodStructures':
            ...
            df = self.parse_calcFolders(elements, self.stage)
        else:
    ...
    def parse_goodStructures(self, elements) -> pd.DataFrame:
        """
        ...
        for table_fp in self.path.iterdir():
            ...
            all_parts.append(df[["id","formula","e_total","natoms","structure","source"]])
    ...
    def parse_calcFolders(
        ...
        return pd.DataFrame(records, columns=["id", "formula", "e_total", "natoms", "structure", "source"])
