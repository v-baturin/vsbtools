...
from ..plotting.formatting import cm2inch
...
def _require_pandas():
    if pd is None:
        raise RuntimeError("Pareto front plotting requires pandas. Install the `materials_dataset` extra.") from _PANDAS_ERROR
    return pd
...
def _read_dataset(manifest_path: Path):
    try:
        ...
        raise RuntimeError(
            "Dataset manifest loading requires pymatgen. Install the `materials_dataset` extra."
        ) from exc
    ...
    *,
    ...
    if n_elements is not None and "composition" in df.columns:
        if _Composition is None:
            raise RuntimeError(
                "Composition filtering requires pymatgen. Install the `materials_dataset` extra."
