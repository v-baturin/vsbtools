...
unixCmd(['echo symmetrization > sym.in']); % title string
unixCmd(['echo ' num2str(tolerance,10) ' accuracy >> sym.in']);
unixCmd(['echo 2    form of lattice parameters: to be entered as lengths and angles >> sym.in']);
unixCmd(['echo ' lattice '   a,b,c,alpha,beta,gamma >> sym.in']);
unixCmd(['echo 1      form of primitive lattice vectors >> sym.in']);
unixCmd(['echo 1 0 0  primitive lattice vectors >> sym.in']);
unixCmd(['echo 0 1 0 >> sym.in']);
unixCmd(['echo 0 0 1  >> sym.in']);
unixCmd(['echo ' num2str(sum(numIons),10) ' number of atoms in the primitive unit cell >> sym.in']);
...
unixCmd(['echo ' s ' type of each atom >> sym.in']);

for coordLoop = 1 : size(coord,1)
  unixCmd(['echo ' num2str(coord(coordLoop,:),11) ' >> sym.in']);
end

[status, nothing] = unix([spgBINDIR '/findsym_new < sym.in > sym.out']);
...
% create a CIF file of the 'symmetrized' structure
  unixCmd('echo end_of_file >> sym.out'); % to easily identify the end of file
  unixCmd(['cat /dev/null > symmetrized.cif']);
  toWrite = 0;
  readNormal = 1;
  currentType = '0';
  typeA = 0;
 ...
 try
  while 1
    ...
    if toWrite
      ...
      tmp = strrep(tmp,'(','\(');
      tmp = strrep(tmp,')','\)');
      unixCmd(['echo ' tmp ' >> symmetrized.cif']);
    end
