...
if exist(fileFullName)
   ...
   if (0==loadFailed) && (1==loadBackFailed)
      unixCmd( ['cp ', fileFullName '        ' , fileFullName '.' '`date +"%b%d-%T"`' ]) ;
      unixCmd( ['cp ', fileFullName '        ' , fileFullName '.backup']);
      USPEXmessage(1001, '', 0);      
   end
   if (1==loadFailed) && (0==loadBackFailed)
      unixCmd( ['cp ', fileFullName '.backup ' , fileFullName '.' '`date +"%b%d-%T"`' ]) ;
      unixCmd( ['cp ', fileFullName '.backup ' , fileFullName]);
      USPEXmessage(1002, '', 0);
   end
   if (loadFailed==1) && (loadBackFailed==1)
      unixCmd(['echo -e "Interupt mat files " >> ERROR'])
      exit;
   end
end
...
unixCmd(['touch ', pathname '/matfileLocker']);
for iC = 1:tryTimes
   saveFailed=0;
   ...
   if saveFailed==1
      if iC < 10
        ...
        unixCmd(['echo -e "Ooops, your file system met a serious problem! USPEX exsiting..." >> Warning.log']);
	% unixCmd(['cp ', fileFullName, '.backup ' , fileFullName ]) ;
         exit;
      end
   else
      copyfile(fileFullName,  [fileFullName, '.backup' ]);
      delete([pathname '/matfileLocker'] );
      return;
   end
end
