...
antiSeedsMax = ORG_STRUC.antiSeedsMax*((sum(fitness(1:fit_len))/fit_len)-min(fitness)); % normalised on fitness variance (sort of)
if (POP_STRUC.generation > ORG_STRUC.antiSeedsActivation + 1) && ~isempty(ANTISEEDS) % limit the minimal possible antiSeedsMax
