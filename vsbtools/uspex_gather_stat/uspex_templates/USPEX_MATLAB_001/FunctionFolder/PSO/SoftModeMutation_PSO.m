function SoftModeMutation_PSO(OFF_ID)
...
%------------------------------------------------------
[POP_ID, numIons, lat, coor, SOFT_ID] = ChooseParent(OFF_ID);
...
for f = good_freq : length(freq)
    ...
    if rand > 0.5  %---SoftMutation: direction 1  
    [good, MUT_LAT, MUT_COORD, deviation] = Do_soft_mutation(coord0, numIons0, lat0, eigV(:,1));
    else           %---SoftMutation: direction 2
    [good, MUT_LAT, MUT_COORD, deviation] = Do_soft_mutation(coord0, numIons0, lat0, -1*eigV(:,1));
    end
%---------------------------------------------------------------------------------
    if good
...
%%%%%%%%%%%%%    
function [POP_ID, numIons, lat, coor, SOFT_ID] = ChooseParent(ind)
global POP_STRUC
global ORG_STRUC
