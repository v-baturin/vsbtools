function addon_diff = KeepBestStructures_001()
...
%  end
 ...
 % - this cycle works wrong for 001 with antiseeds
     
 for num_kb = 1: keepBestHM % num_kb structures will be chosen as keptBest structurs
     addon = 0; %number of strcuture in ranked population
     num_cur = 0; % current number of structure to be chosen as keptBest
     while num_cur < num_kb
         addon = addon + 1;
         flag = 0;
         for i = 1 : size(POP_STRUC.convex_hull,1)
             ind_ch = POP_STRUC.convex_hull(i,end);
             ind_cand = POP_STRUC.ranking(addon);
             if isequal(ind_ch, ind_cand) || SameStructure(ind_ch, ind_cand, POP_STRUC)
                 flag = 1; % this structure is on convex hull
                 break;
             end
         end
         if flag == 0
