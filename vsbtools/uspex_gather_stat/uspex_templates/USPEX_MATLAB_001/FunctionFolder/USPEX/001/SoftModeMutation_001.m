...
for j = 1 : length(POP_STRUC.SOFTMODEParents)
    if isequal(numIons, POP_STRUC.SOFTMODEParents(j).numIons)
...
%%001mode
% if (flag~=1) || ((flag == 1) && (POP_STRUC.SOFTMODEParents(j).Softmode_num <= length(freq)))
%    break;
% end
% end
%%001mode
      ...
      if loop==1
         break
          ...
          for i = 0 : 10
              ...
              if goodAtomMutant == 1
                 goodAtomMutant = CheckOldOffspring(MUT_LAT, MUT_COORD, numIons, Ind_No, 'SoftModeMutation_001');
              end
