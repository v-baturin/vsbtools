...
init_repet = struct('Heredity_001', 0, 'Random_001', 0,...
                   'Permutation_001', 0, 'AddAtom_001', 0, 'RemoveAtom_001', 0,...
                    'Transmutation_001', 0, 'SoftModeMutation_001', 0, 'Mutation_001', 0);
CLUSTERS.repetitions = init_repet; %Counters of duplicating structures created by different operators
CLUSTERS.Structure =  struct('coord',{},'numIons',{},'lattice',{}, 'enthalpy',{}, 'fingerprint',{},'Atom',{});
...
elseif size(ORG_STRUC.numIons,2) == 3  % two-component clusters
        for i = 1 : ORG_STRUC.numIons(2,1) - ORG_STRUC.numIons(1,1) + 3
	    for j = 1 : ORG_STRUC.numIons(2,2) - ORG_STRUC.numIons(1,2) + 3
        for k = 1 : ORG_STRUC.numIons(2,3) - ORG_STRUC.numIons(1,3) + 3
            	CLUSTERS.composition(i,j,k).bestEnthalpy = 10000;
            	CLUSTERS.composition(i,j,k).numIons = [i + ORG_STRUC.numIons(1,1) - 2, j + ORG_STRUC.numIons(1,2) - 2, k + ORG_STRUC.numIons(1,3) - 2];
        end
        end
        end
end
...
elseif size(ORG_STRUC.numIons,2) == 3  % three-component clusters
   num1 = (ORG_STRUC.numIons(2,1) - ORG_STRUC.numIons(1,1) + 1);
   num2 = (ORG_STRUC.numIons(2,2) - ORG_STRUC.numIons(1,2) + 1);
   num3 = (ORG_STRUC.numIons(2,3) - ORG_STRUC.numIons(1,3) + 1);
   numComp = num1*num2*num3;
   curComp = 1 + round((numComp-1)*(goodPop-1)/(ORG_STRUC.initialPopSize-1));
   
   numIons(1) = ORG_STRUC.numIons(1,1) + fix((curComp-1) / (num2*num3));
   delta_num2 = curComp - (numIons(1)-ORG_STRUC.numIons(1,1))*num2*num3;
   numIons(2) = ORG_STRUC.numIons(1,2) + fix((delta_num2-1)/num3);
   delta_num3 = delta_num2 - (numIons(2)-ORG_STRUC.numIons(1,2))*num3;
   numIons(3) = ORG_STRUC.numIons(1,3) + delta_num3-1;
end
...
tic
          ...
          POP_STRUC.POPULATION(goodPop).INIT_FP = ...
                    fp_calc_001(lat, candidate, numIons);
