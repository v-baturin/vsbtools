...
elseif size(ORG_STRUC.numIons,2) == 3  % three-component clusters
  numIons(1) = ORG_STRUC.numIons(1,1) + round((ORG_STRUC.numIons(2,1) - ORG_STRUC.numIons(1,1)) * rand(1));
  numIons(2) = ORG_STRUC.numIons(1,2) + round((ORG_STRUC.numIons(2,2) - ORG_STRUC.numIons(1,2)) * rand(1));
  numIons(3) = ORG_STRUC.numIons(1,3) + round((ORG_STRUC.numIons(2,3) - ORG_STRUC.numIons(1,3)) * rand(1));
end
  ...
  if errorS == 0
    ...
    if goodStructure == 1
       goodStructure = CheckOldOffspring(lat, candidate, numIons, Ind_No, 'Random_001');
    end
