function [numIons, offspring,potentialLattice,fracFrac,dimension,offset,fracLattice] = h1eredity_cluster(parent1,parent2,parentlat1,parentlat2, order1, order2, numIons1, numIons2, fracFrac)
...
for ind = 1:length(ORG_STRUC.atomType)
    ...
    minIons = min(numIons1(ind),numIons2(ind));
    maxIons = max(numIons1(ind),numIons2(ind));
    
    if ionCount(ind,1) < minIons  %ORG_STRUC.numIons(1,ind)   %numIons(ind)     %%%001mode - binary clusters
        numIons(ind) = minIons;  %ORG_STRUC.numIons(1,ind);                      %%%001mode - binary clusters

        % calculate how many ions (of this type) are missing
        howmany = minIons - ionCount(ind,1);		%%001mode - binary clusters
    ...
    elseif  ionCount(ind,1)>maxIons  %ORG_STRUC.numIons(2,ind)         %%%001mode - binary clusters
        numIons(ind) = maxIons;  %ORG_STRUC.numIons(2,ind);             %%%001mode - binary clusters

        howmany = ionCount(ind,1) - maxIons;  %%001mode - binary clusters
