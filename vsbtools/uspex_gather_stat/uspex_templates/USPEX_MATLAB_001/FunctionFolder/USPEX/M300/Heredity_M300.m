...
while searching
   ...
   while goodHeritage ~=1
         ...
         [GB_numIons, GB_candidate, GB_lat, fracFrac, dimension ] = ...
                                      heredity_Make_M300(ind1,ind2);
         
         numIons = GB_numIons + bulk_numIons;
         item = 0;
         GB_atyp = [];
         for i = 1:length(GB_numIons)
             GB_atyp(1,item+1:item+GB_numIons(i))=atomType(i);
             item = item + GB_numIons(i);
         end
         [lat,candidate, typesAList, chanAList] = makeGB(...
                  numIons, GB_lat, GB_candidate, GB_atyp,...
               bulk_lat, bulk_pos, bulk_atyp, vacuumSize);

         goodHeritage = distanceCheck(candidate, lat, numIons, minD);
