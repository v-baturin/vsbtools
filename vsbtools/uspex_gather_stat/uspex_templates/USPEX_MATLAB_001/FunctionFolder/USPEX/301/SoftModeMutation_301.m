function SoftModeMutation_301(OFF_ID)
...
global ORG_STRUC
global OFF_STRUC
%This is very similar to SoftModeMutation_300, except
%1, pick structure from POOL instead of POP
%2, Grey fingerprint is used here (sum(numIons))
...
POP_ID = chooseGoodComposition(ORG_STRUC.tournament, ORG_STRUC.paretoRanking, POOL_STRUC);
f1     = POOL_STRUC.POPULATION(POP_ID).FINGERPRINT;
...
if SOFT_ID == 0
    numIons = POOL_STRUC.POPULATION(POP_ID).numIons;
    lat     = POOL_STRUC.POPULATION(POP_ID).LATTICE;
    coor    = POOL_STRUC.POPULATION(POP_ID).COORDINATES;
...
OFF_STRUC.POPULATION(OFF_ID).numBlocks   = POOL_STRUC.POPULATION(POP_ID).numBlocks;
OFF_STRUC.POPULATION(OFF_ID).howCome     = 'softmutate';

info_parents = struct('parent', {},'mut_degree', {}, ...
     'mut_vec',{},'mut_mode',{},'mut_fre',{},'enthalpy',{});
info_parents(1).parent  = num2str(POOL_STRUC.POPULATION(POP_ID).Number);
N_atom = sum(POOL_STRUC.POPULATION(POP_ID).numIons);
info_parents.enthalpy   = POOL_STRUC.POPULATION(POP_ID).enthalpy;
...
[Ni, V, dist_matrix, typ_i, typ_j] = makeMatrices(...
            MUT_LAT0, MUT_COORD0, sum(numIons0), 1);
[order1, f1, atom_fing1] = fingerprint_calc(...
            Ni, V, dist_matrix, typ_i, typ_j, sum(numIons0));
goodfreq = lastfreq + 1;
...
[Ni, V, dist_matrix, typ_i, typ_j] = makeMatrices(MUT_LAT0, MUT_COORD0, sum(numIons0), 1);
[order1, f1, atom_fing1]           = fingerprint_calc(Ni, V, dist_matrix, typ_i, typ_j, sum(numIons0));

[MUT_LAT0, MUT_COORD0, deviation0] = move_along_SoftMode_Mutation(coord0, numIons0, lat0, -1*eigV, 1);
[Ni, V, dist_matrix, typ_i, typ_j] = makeMatrices(MUT_LAT0, MUT_COORD0, sum(numIons0), 1);
[order1, f2, atom_fing1]           = fingerprint_calc(Ni, V, dist_matrix, typ_i, typ_j, sum(numIons0));
