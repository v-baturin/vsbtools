...
x = zeros(sum(accepted(:)),size(numIons,1)-1);   
y = zeros(sum(accepted(:)),1);
x_= zeros(sum(newInd(:)),  size(numIons,1)-1);
...
unixCmd(['cat /dev/null > extended_convex_hull_POSCARS']);  %Start to get new POSCAR
