...
howManyTrans = round(howManySwaps*sum(numBlocks));
...
if ~(specificSwaps == 0)
 ...
 for i = 1 : length(numBlocks)
   if (~isempty(find(i == specificSwaps))) && (numBlocks(i) > 0)
    validStr = 1;
   end
 end
 if validStr == 0
...
for any = 1 : transNow
  ...
  % TO DO: add minIon and maxIon contrains!
     ...
     while specified
       whichTypeFirst = RandInt(1,1,[1,length(numBlocks)]);
       if (~isempty(find(whichTypeFirst == specificSwaps))) && (numBlocks(whichTypeFirst) > 0)
          [row, col] = find(whichTypeFirst == specificSwaps);
          acceptedTrans = [];
          for counting = 1 : length(row)
             if col(counting)-0.1 > 1
               acceptedTrans = cat(1,acceptedTrans,specificSwaps(row(counting),1:col(counting)-1));
             end
             if col(counting)+0.1 < size(specificSwaps,2)
                acceptedTrans = cat(1,acceptedTrans,specificSwaps(row(counting),col(counting)+1:end));
             end
          end
         specified = 0;
       end
