...
if ORG_STRUC.optType == 1 % enthalpy
    ...
    for i = 1:length(POP_STRUC.POPULATION)
	    ...
	    vol     = det(POP_STRUC.POPULATION(i).LATTICE);
            if ORG_STRUC.optType == 2 % volume
	        fitness(i) = vol/sum(numIons);
		...
		fitness(i) = -1*calcDensity(numIons, ORG_STRUC.atomType, vol);
...
%% Dong Dong
if ORG_STRUC.optType == 14 % thermoelectric property, replaced power factor
    for fit_loop = 1 : length(POP_STRUC.POPULATION)
...
%%--- ZX: to store distances from convex hull in USPEX_STRUC.POPULATION.Dis_ConvexHull
if (abs(ORG_STRUC.optType) ~= 4)  &  (abs(ORG_STRUC.optType) ~=5)
