function ConVeX = distance_convex_hull(numIons, homePath)
...
N_Point   = length(USPEX_STRUC.POPULATION);
numBlocks = zeros(N_Point, N_Block);
Energy    = zeros(N_Point, 1);

for i=1:N_Point
    numIONS(i,:)   = USPEX_STRUC.POPULATION(i).numIons;
    numBlocks(i,:) = USPEX_STRUC.POPULATION(i).numBlocks;
    if (USPEX_STRUC.POPULATION(i).numIons == (numBlocks(i,:) * numIons))
        coef  = 1;
    else
        coef = USPEX_STRUC.POPULATION(i).numIons/(numBlocks(i,:) * numIons);
	numBlocks(i,:) = round(coef * numBlocks(i,:));
    end
    Energy(i)      = USPEX_STRUC.POPULATION(i).Enthalpies(end)/sum(numBlocks(i,:));  % eV/Block
end
...
for i = 1 : N_Point
    fitness(i) = fitness(i) * sum(numBlocks(i,:)) / sum(numIONS(i,:));
    USPEX_STRUC.POPULATION(i).Dis_ConvexHull = fitness(i);
end
ConVeX = fitness;
safesave([homePath,'/',resFolder '/USPEX.mat'], USPEX_STRUC);
