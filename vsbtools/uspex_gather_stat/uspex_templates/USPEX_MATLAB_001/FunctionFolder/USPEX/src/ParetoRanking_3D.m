function [Stru_Num, Distance, ParetoFront] = ParetoRanking_3D(f1, f2, f3, St_Num)
% Main_F
...
while size(main_tmp,1) > 0
	...
	f2 = main_tmp(a,3);
	f3 = main_tmp(a,4);
	if firstParetoFront(f1, f2, f3, main_tmp, a) == 1
	    Counter = Counter +1;
	    main_tmp(a,5) = ParetoCounter;
...
for a = 1 : size(new_tmp,1)
    new_tmp(a,6) = sqrt((new_tmp(a,2) ^ 2) + (new_tmp(a,3) ^ 2) + (new_tmp(a,4) ^ 2));    % distance
end

%%% Ordering structures according to their pareto front, 
for ParFront = 1 : new_tmp(end,5)    % for a = 1 : last pareto front
    n = 1;
    draft = [];
    for i = 1 : size(new_tmp,1)
	if new_tmp(i,5) == ParFront
	    draft(n,:) = new_tmp(i,:);
	    n = n + 1;
	end
    end
...
%%%%%------------------------------------------------
function result = firstParetoFront(f1, f2, f3, main_tmp, ID)
result = 1;
main_tmp(ID,:) = [];
for a = 1 : size(main_tmp,1)
    if (f1 == main_tmp(a,2)) && (f2 == main_tmp(a,3)) && (f3 == main_tmp(a,4))

    elseif (f1 >= main_tmp(a,2)) && (f2 >= main_tmp(a,3)) && (f3 >= main_tmp(a,4))
        result = 0;
        break;
    end
end
