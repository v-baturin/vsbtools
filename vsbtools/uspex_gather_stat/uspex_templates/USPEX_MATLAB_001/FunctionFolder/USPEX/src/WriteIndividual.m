...
for i=1:length(USPEX_STRUC.POPULATION)
    ...
    enth     = USPEX_STRUC.POPULATION(i).Enthalpies(ORG_STRUC.conv_till);
    num      = USPEX_STRUC.POPULATION(i).numIons;
    fit      = USPEX_STRUC.POPULATION(i).Fitness;
    howcome  = USPEX_STRUC.POPULATION(i).howCome;
    KPOINTS  = USPEX_STRUC.POPULATION(i).K_POINTS(ORG_STRUC.conv_till,:);
...
if ORG_STRUC.spin == 1
    fprintf(fp,['%3d %4d %-11s '  comp_format ' %9.3f ' volume_format ' ' density_format ' %10.3f ' kpoints_format ' ' symmetry_format ' ' entropy_format ' ' ao_format ' ' s_format  ' ' mag_format ' ' magType_format '\n'], ...
                 gen, i, howcome, composition,  enth,   volume,           density,         fit,     KPOINTS(:),        symg,               entropy,           a_o,          s,  magmom, magTypeString(magType(1)) );
else
