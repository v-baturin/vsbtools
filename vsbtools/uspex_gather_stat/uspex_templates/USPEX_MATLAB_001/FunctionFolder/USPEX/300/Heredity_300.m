    ...
    if (ORG_STRUC.manyParents == 0) || (ORG_STRUC.manyParents > 1)
        ...
        while same
            if ORG_STRUC.paretoRanking ~= 0
                ...
                par_one = randi(POP_STRUC.paretoFront(ParetoF(end))) + sum(POP_STRUC.paretoFront(1 : ParetoF(end) - 1));
                ParetoF = find (ORG_STRUC.tournament > randi([0,max(ORG_STRUC.tournament)-1]));
                %%All structures in same paretofront have the same possibility to get chosen!!
                par_two = randi(POP_STRUC.paretoFront(ParetoF(end))) + sum(POP_STRUC.paretoFront(1 : ParetoF(end) - 1));
	    else
