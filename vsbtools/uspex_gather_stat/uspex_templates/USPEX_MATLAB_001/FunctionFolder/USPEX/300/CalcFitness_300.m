...
elseif ORG_STRUC.optType == 5 % density
    for fit_loop = 1:length(POP_STRUC.POPULATION)
	Volume  = det(POP_STRUC.POPULATION(fit_loop).LATTICE);
	numIons = POP_STRUC.POPULATION(fit_loop).numIons;
        fitness(fit_loop) = -1*calcDensity(numIons, ORG_STRUC.atomType, Volume);
    end
...
elseif ORG_STRUC.optType == 9
    for fit_loop = 1 : length(POP_STRUC.POPULATION)
        Volume  = det(POP_STRUC.POPULATION(fit_loop).LATTICE);
        fitness(fit_loop) = -1*POP_STRUC.POPULATION(fit_loop).mag_moment/Volume;
    end
...
elseif ORG_STRUC.optType == 11 % birefringence
    for fit_loop = 1 : length(POP_STRUC.POPULATION)
        fitness(fit_loop) = -1*POP_STRUC.POPULATION(fit_loop).birefringence;
    end
elseif ORG_STRUC.optType == 14 % thermoelectric property, replaced power factor
    for fit_loop = 1 : length(POP_STRUC.POPULATION)
        fitness(fit_loop) = -1*POP_STRUC.POPULATION(fit_loop).TE_property;
    end
elseif ORG_STRUC.optType == 17 % Gibbs free energy using SCPH
   for fit_loop = 1 : length(POP_STRUC.POPULATION)
       fitness(fit_loop) = POP_STRUC.POPULATION(fit_loop).Enthalpies(end)/ORG_STRUC.numIons + POP_STRUC.POPULATION(fit_loop).Fphon(end);
   end
