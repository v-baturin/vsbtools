...
if sum(ORG_STRUC.bulk_ntyp >0) == 1   %substrate is elemental
     if length(ORG_STRUC.numIons) == 1 % only one type of surface atom(eg: C on C(111))
          ...
          for i=1:length(POP_STRUC.POPULATION)
                 N_cell = det(reshape(POP_STRUC.POPULATION(i).cell,[2,2]));
                 fitness(i)=POP_STRUC.POPULATION(i).Enthalpies(end)-ORG_STRUC.E_AB*N_cell;
...
title(['E_{formation} = E - N_{' B '} E_{AB}/n - (N_{' A '} - m N_{' B '}/n) \mu_{' A '}']);
xlabel(['\mu(' A ')']);
end
...
for i=1:size(POP_STRUC.convex_hull, 1)
   if POP_STRUC.convex_hull(i,3) > 0
       ...
       N_cell = det(reshape(POP_STRUC.POPULATION(Ind).cell,[2,2]));
