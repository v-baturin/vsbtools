...
try
    unixCmd(['cat qEspresso_options_' num2str(Step) ' > qe.in']);
    unixCmd(['sed -e "s/AAAA/' num2str(sum(numIons)) '/" qe.in > TEMP']);
    unixCmd(['sed -e "s/BBBB/' num2str(length(numIons)) '/" TEMP > qe.in']);
    delete('TEMP');
catch
