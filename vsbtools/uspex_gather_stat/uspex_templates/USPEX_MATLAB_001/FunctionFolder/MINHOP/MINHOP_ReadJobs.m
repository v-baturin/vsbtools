...
for indic = 1:ORG_STRUC.numParallelCalcs
    ...
    if ~isempty (whichInd)
       ...
       if POP_STRUC.POPULATION(whichInd).JobID
           ...
           if doneOr
              MINHOP_Reading(ORG_STRUC.abinitioCode(Step),whichInd, indic);
              ...
              elseif Step == maxStep
                  ...
                  if POP_STRUC.finalOptimization == 0
                     ...
                     MINHOP_WriteIndividualOutput(whichInd, Step, 1);  %WriteIndividuals before full relaxation
                  else

                     MINHOP_WriteIndividualOutput(whichInd, Step, 2);  %WriteIndividuals after  full relaxation
                     disp('local relaxation is done.')
                  end
                  POP_STRUC.POPULATION(whichInd).Folder=0;
                  POP_STRUC.POPULATION(whichInd).Done = 1;
                  POP_STRUC.POPULATION(whichInd).ToDo = 0;
